/**
 * Генерация заголовков по НЧ-запросам (Промпт 1). Количество задаётся в Настройках (Макс. заголовков).
...
export async function generateHeadlines(
  ...
  let items = parseHeadlinesWithKeywords(content, keywordList, count);
  ...
  if (items.length === 0) {
    const headlinesLegacy = parseHeadlinesLegacy(content, count);
