...
| Справочник каталога | Ссылка на Google Docs: строки «Раздел\tURL» или **таблица** (колонки с названием раздела и ссылкой) — для UTM-ссылок |
...
| Шаблон UTM | Плейсхолдеры: `{topic}`, `{campaign}` — slug заголовка; `{keyword}` — ключевое слово задачи. Пример: `?utm_source=dzen&utm_medium=organic&utm_campaign=content_zavod&utm_content={keyword}` |
| Время генерации | Время суток (например, `05:00`), после которого запускается генерация **картинки** для строк со статусом «Текст готов, ждём картинку». Текст генерируется сразу после выбора заголовка (статус «Согласован заголовок»). Пусто — по умолчанию 05:00. |
| Интервал публикации (мин) | Минуты между публикациями одобренных статей (например, `60`). По умолчанию 60. |
| Telegram Channel ID, Макс. статей в день, Режим модерации, Время сводки | Остальные настройки |

### Статусы заданий (лист «Задания»)

| Статус | Действие |
|--------|----------|
| **Перегенерировать картинку** | Перегенерация только изображения по текущему заголовку и Справочнику фото, без пересборки текста. Обновляются колонки Картинка, Стоимость картинки, Итого; статус переводится в «Готово к проверке». |
